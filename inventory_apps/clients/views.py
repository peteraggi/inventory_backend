...
from .serializers import OnboardingSerializer, SubscriptionPlanSerializer, ClientDetailSerializer
...
def _seed_tenant(schema_name, store_name, owner_name, owner_email, owner_password, owner_phone):
    """
    Called inside schema_context. Creates default roles, a first store,
    and the owner account, then returns JWT tokens for the owner.
    """
    from inventory_apps.pos_app.models import Store, Role
...
class OnboardingAPIView(views.APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description=(
            ...
            'on `{subdomain}.api.inventory.com`.\n\n'
            '**`plan`** defaults to `"free"` (30-day trial). '
            '**`store_name`** defaults to `business_name` if omitted.'
        ),
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            required=['business_name', 'contact_name', 'contact_email', 'password', 'subdomain'],
            properties={
                'business_name': openapi.Schema(type=openapi.TYPE_STRING, example='Acme Retail'),
                'contact_name':  openapi.Schema(type=openapi.TYPE_STRING, example='John Doe'),
                'contact_email': openapi.Schema(type=openapi.TYPE_STRING, format='email', example='john@acme.com'),
                'contact_phone': openapi.Schema(type=openapi.TYPE_STRING, example='08012345678'),
                'password':      openapi.Schema(type=openapi.TYPE_STRING, format='password', example='secret123'),
                'subdomain':     openapi.Schema(type=openapi.TYPE_STRING, example='acme',
                                                description='3–63 chars, letters/digits/hyphens. Becomes acme.api.inventory.com'),
                'plan':          openapi.Schema(type=openapi.TYPE_STRING, enum=['free', 'basic', 'standard', 'enterprise'],
                                                default='free'),
                'store_name':    openapi.Schema(type=openapi.TYPE_STRING, example='Acme HQ'),
            },
        ),
        responses={
                    ...
                    properties={
                        ...
                        'tenant': openapi.Schema(
                            ...
                            properties={
                                ...
                                'api_base_url':  openapi.Schema(type=openapi.TYPE_STRING),
                                'plan':          openapi.Schema(type=openapi.TYPE_STRING),
                                'on_trial':      openapi.Schema(type=openapi.TYPE_BOOLEAN),
                                'trial_ends':    openapi.Schema(type=openapi.TYPE_STRING, format='date'),
                            },
                        ),
            ...
            400: openapi.Response('Validation error (e.g. subdomain taken)', schema=_ERROR_SCHEMA),
    ...
    def post(self, request):
        ...
        full_domain = f'{subdomain}.api.inventory.com'
